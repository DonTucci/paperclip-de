    ...
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-border p-4">
          ...
          <div className="mt-2">
            <ToggleSwitch checked={on} disabled={toggleDisabled} onCheckedChange={onToggle} aria-label={tf("auto.10748b999fbf84ff")} />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">{tf("auto.a27be6847d5dc2a6")}</p>
        </div>
        <StatCard label={tf("auto.89dd748442c19485")}>
          {apps.length} {apps.length === 1 ? "app" : "apps"}
          {profile ? ` · ${allowedToolsLabel(profile)}` : ""}
        </StatCard>
        <StatCard label={tf("text.Tokens")}>
          {active} active{expiring > 0 ? ` · ${expiring} expiring` : ""}
        </StatCard>
        <StatCard label={tf("text.Health")}>
      ...
      <section className="rounded-lg border border-border p-4">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            <h3 className="text-sm font-semibold text-foreground">{tf("auto.82d9af69e1a18656")}</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {tf("auto.bd0cff72ee94276c")}
            </p>
          </div>
        </div>
      ...
      <section className="rounded-lg border border-border p-4">
        <h3 className="text-sm font-semibold text-foreground">{tf("auto.f79f93650e91b185")}</h3>
        {apps.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">
            {tf("auto.de912bf377c1b86d")}
          </p>
        ) : (
      ...
      <section className="rounded-lg border border-border bg-muted/30 p-4">
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-semibold text-foreground">{tf("auto.0b9f00af0f2ab0b4")}</h3>

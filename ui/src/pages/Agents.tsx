...
export function Agents({ initialView = "list" }: { initialView?: AgentsView } = {}) {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={tf("auto.160f63a71271f783")} />;
  }
    ...
    <div className={cn(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {!forceListView ? <div className="flex items-center overflow-hidden rounded-md border border-border" role="group" aria-label={tf("auto.c26e47e839f072c8")}>
              <Button
                type="button"
                size="icon-sm"
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {tf(filtered.length === 1 ? "agents.count_one" : "agents.count_other", { count: filtered.length })}
        </p>
      )}

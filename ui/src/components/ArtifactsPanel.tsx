...
const FILTERS: Array<{ label: string; value: FilterValue }> = [
  { label: tf("text.All"), value: "all" },
  { label: tf("auto.b4cc4b07c300103a"), value: "in_progress" },
  { label: tf("auto.b86832322d80814b"), value: "for_review" },
  { label: tf("auto.22a970d2e5b1cc23"), value: "completed" },
];
...
function statusBadge(status: string) {
  switch (status) {
    case "active":
    case "draft":
      return { label: tf("auto.b4cc4b07c300103a"), className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { label: tf("auto.b86832322d80814b"), className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { label: tf("auto.22a970d2e5b1cc23"), className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { label: tf("auto.36f684a51c88f921"), className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { label: tf("text.Failed"), className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{tf("text.Artifacts")}</h3>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            {tf("auto.f651afe11c137eef")}
          </div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">{tf("auto.f97a51c62e53d670")}</p>
        ) : doc?.body ? (
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-(length:--text-micro) text-muted-foreground mb-2">{tf("auto.ba12212b60a47ba4")}</p>

...
export function ProjectRepositories({ project }: { project: Project }) {
  ...
  return <section aria-label={tf("text.Repositories")} className="flex min-w-0 flex-col gap-4 py-4">
    ...
    <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border pt-4">
      ...
      {save.isSuccess && <span role="status" className="mr-auto text-sm text-muted-foreground">{tf("auto.16bf144aa29f54ed")}</span>}
      {draft && <Button type="button" variant="ghost" disabled={save.isPending} onClick={() => { setDraft(null); save.reset(); }}>{tf("auto.f9bfa3dc7c8fcdea")}</Button>}

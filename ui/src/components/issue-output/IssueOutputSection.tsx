...
export function IssueOutputSection({ workProducts, resolveCreatorName, onMediaClick }: IssueOutputSectionProps) {
  ...
  return (
    <section className="space-y-3" aria-label={tf("auto.9e5b285b313baa9f")}>
      <div className="flex items-center gap-2">
        <Play className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        <h3 className="text-sm font-medium text-muted-foreground">{tf("text.Output")}</h3>
      ...
      {rest.length > 0 ? (
        <div className="space-y-2">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{tf("auto.a17e2163cb177831")}</p>
          {mediaRest.length > 0 ? (

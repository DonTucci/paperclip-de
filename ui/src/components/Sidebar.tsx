    ...
    >
      ...
      <nav className={primarySidebarStyles.nav}>
        <div className={primarySidebarStyles.group}>
          ...
          {(() => {
            const newTaskButton = (
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{tf("nav.newTask")}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{tf("nav.newTask")}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          <SidebarNavItem to="/search" label={tf("nav.search")} icon={Search} />
          <SidebarNavItem to="/dashboard" label={tf("nav.dashboard")} icon={LayoutDashboard} liveCount={liveRunCount} />
          <SidebarNavItem
            to="/inbox"
            label={tf("nav.inbox")}
            icon={Inbox}
          ...
          {showStatusCards ? (
            <SidebarNavItem to="/status" label={tf("nav.status")} icon={LayoutGrid} textBadge="beta" />
          ) : null}
        ...
        <SidebarSection label={tf("nav.work")} collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          <SidebarNavItem to="/issues" label={tf("nav.tasks")} icon={CircleCheck} />
          {streamlinedUiEnabled ? (
            <>
              <SidebarNavItem to="/projects" label={tf("nav.projects")} icon={FolderOpen} />
              <SidebarStarredProjects />
            </>
          ) : null}
          <SidebarNavItem to="/routines" label={tf("nav.routines")} icon={Repeat} />
          <SidebarNavItem to="/artifacts" label={tf("nav.artifacts")} icon={Package} />
          {showCases ? (
          ...
          {showGoalsLink ? (
            <SidebarNavItem to="/goals" label={tf("nav.goals")} icon={Target} />
          ) : goalsLinkPending ? (
            <div
          ...
          {showWorkspacesLink ? (
            <SidebarNavItem to="/workspaces" label={tf("nav.workspaces")} icon={GitBranch} />
          ) : null}
          <PluginSlotOutlet
            slotTypes={["sidebar"]}
        ...
        {streamlinedUiEnabled ? (
          ...
          >
            <SidebarNavItem to="/agents" label={tf("nav.agents")} icon={Users} />
            <SidebarNavItem to="/skills" label={tf("nav.skills")} icon={Boxes} />
            <SidebarNavItem to="/apps" label={tf("nav.connectors")} icon={Unplug} />
            <SidebarNavItem to="/activity" label={tf("nav.audit")} icon={History} />
          </SidebarSection>
        ) : null}
        ...
        {streamlinedUiEnabled ? (
          ...
          <>
            ...
            <SidebarAgents />
            <SidebarSection
              label={tf("nav.organization")}
              collapsible={{ open: organizationOpen, onOpenChange: setOrganizationOpen }}
            >
              <SidebarNavItem to="/org" label={tf("nav.org")} icon={Network} />
              <SidebarNavItem to="/apps" label={tf("nav.connectors")} icon={Unplug} />
              <SidebarNavItem to="/timeline" label={tf("nav.timeline")} icon={GanttChartSquare} />
              <SidebarNavItem to="/costs" label={tf("nav.costs")} icon={DollarSign} />
              <SidebarNavItem to="/activity" label={tf("nav.activity")} icon={History} />
              <SidebarNavItem to="/company/settings" label={tf("nav.settings")} icon={Settings} />
            </SidebarSection>
          </>
        )}

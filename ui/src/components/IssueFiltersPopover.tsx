      ...
      >
        <div className="space-y-3 p-3">
            ...
            <>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="space-y-1" data-filter-options="inbox-category">
                  <span className="text-xs text-muted-foreground">{tf("text.Category")}</span>
                ...
                {inboxScopeFilters.showApprovalStatus ? (
                  <div className="space-y-1" data-filter-options="inbox-approval-status">
                    <span className="text-xs text-muted-foreground">{tf("auto.5a66175a2ba3931a")}</span>
                    <div className="space-y-0.5">
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{tf("auto.e01c5f270819f8ff")}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                ...
                {streamlined && (agents?.length ?? 0) + (currentUserId ? 2 : 1) > SEARCHABLE_FILTER_THRESHOLD ? (
                  <FilterOptionSearch value={assigneeSearch} onChange={setAssigneeSearch} label={tf("auto.bc110a6d0722098a")} />
                ) : null}
                <div data-filter-options="responsible" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{tf("auto.15abdee5d453108e")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{tf("auto.d30af076b0dc85cc")}</span>
                    </label>
                  ) : null}
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{tf("auto.88447b83090cded5")}</span>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={tf("auto.2294896cefc24368")}
                      className="h-8 pl-7 text-xs"
                    />
                  </div>
                  <div data-filter-options="creators" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{tf("auto.e23e5a42b5572904")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{tf("text.Project")}</span>
                  {streamlined && projects.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={projectSearch} onChange={setProjectSearch} label={tf("text.Projects")} />
                  ) : null}
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{tf("text.Labels")}</span>
                  {streamlined && labels.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={labelSearch} onChange={setLabelSearch} label={tf("text.Labels")} />
                  ) : null}
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{tf("text.Workspace")}</span>
                  {streamlined && workspaces.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={workspaceSearch} onChange={setWorkspaceSearch} label={tf("text.Workspaces")} />
                  ) : null}
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{tf("auto.48bc2014d5fb4ce5")}</span>
                  <div className="space-y-0.5">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{tf("auto.7448611d5f93aa8a")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{tf("auto.596632fc019ac3ae")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{tf("auto.d4a9dd19a5871e9f")}</span>
                  </label>
                ) : null}
              </div>
            </div>

...
export function DecisionQueueRail({ companyId, activeQueueKey = null }: DecisionQueueRailProps) {
  ...
  return (
    <nav className="flex flex-wrap items-center gap-1.5" aria-label={tf("auto.6668ff49dc54ac93")} data-decision-queue-rail>
      <Chip href={decisionsHref(null)} active={activeQueueKey == null} label={tf("text.All")} />
      {queues.map((queue) => {
...
function Chip({
  ...
  return (
    ...
    >
      {recent && (
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-primary" aria-label={tf("auto.6cb44b56336af70b")} />
      )}

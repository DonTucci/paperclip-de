...
export function IssueDocumentsSection({
  ...
  const deleteDocument = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message: tf("auto.62de8fe94b0bf271"));
    },
  });
  ...
  const restoreDocumentRevision = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message: tf("auto.1e5055596515e0b9"));
    },
  });
  ...
  const setDocumentLock = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message: tf("auto.9ed8dbcf52210e61"));
    },
  });
  ...
  const commitDraft = useCallback(async (
    ...
    try {
      ...
      setError(err instanceof Error ? err.message: tf("auto.210b653bb478a0cd"));
      return false;
    }
      ...
      {isEmpty && !draft?.isNew ? (
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          ...
          <Button variant="outline" size="sm" onClick={beginNewDocument} className="shrink-0">
            ...
            <span className="hidden sm:inline">{tf("auto.1b26844054631e28")}</span>
            <span className="sm:hidden">{tf("auto.18fdd549b2ed367a")}</span>
          </Button>
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-2 min-w-0">
          <h3 className="w-full text-sm font-medium text-muted-foreground shrink-0 sm:w-auto">{tf("text.Documents")}</h3>
      ...
      {draft?.isNew && (
        ...
        >
          <Input
            ...
            placeholder={tf("auto.75622cdf9465d1a6")}
          />
          {newDocumentKeyError && (
          ...
          {!isPlanKey(draft.key) && (
            <Input
              ...
              placeholder={tf("auto.67fc5f45e20b3338")}
            />
          )}
          <MarkdownEditor
            value={draft.body}
            ...
            placeholder={tf("auto.4ddfc15ba3f746ed")}
            bordered={false}
                ...
                actionsSlot={
                  <>
                    ...
                    <Button
                      ...
                      title={isLocked ? tf("auto.a1cf639c9e9d0706") : tf("auto.9950ef893ee82593")}
                    ...
                    >
                      ...
                      <span title={tf("auto.b09d58e20fc02ffe")} aria-label={tf("auto.b09d58e20fc02ffe")} className="inline-flex h-6 w-6 items-center justify-center text-amber-700 dark:text-amber-300">
                    ...
                    <Button
                      ...
                      title={copiedDocumentKey === doc.key ? tf("text.Copied") : tf("auto.c054131e44515b54")}
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          title={tf("auto.97fa0725b718c1b9")}
                        >
                ...
                >
                  ...
                  {activeConflict && !isHistoricalPreview && (
                    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-3">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">{tf("auto.c8f1506a7d60a32e")}</p>
                          <p className="text-xs text-muted-foreground">
                            {tf("auto.fcbbd08cf5e60bed")}
                          </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          <Button

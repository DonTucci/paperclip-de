  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="h-8 gap-1 text-xs font-normal" aria-label={tf("auto.765b3c6a932f9505")}>
          <ArrowUpDown className="h-3.5 w-3.5 text-muted-foreground" aria-hidden />
          <span className="hidden sm:inline text-muted-foreground">{tf("auto.8f3e7ea6ed491d4f")}</span>
          <span>{SORT_LABELS[value]}</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel className="text-xs text-muted-foreground">{tf("auto.c9129025bd3ff652")}</DropdownMenuLabel>
        <DropdownMenuSeparator />

...
export function AiConnectionAccountControls({
  ...
  return (
    <section className="space-y-4" aria-label={tf("auto.da99238211bc1c75")}>
      {ownPersonal && (
        <div className={cn(
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold">{tf("auto.a74362e5c5612aa8")}</h3>
          ...
          {account.isDefault ? (
            ...
            <Button variant="outline" size="sm" disabled={!available} onClick={onMakeDefault}>{tf("auto.f43b94251094a888")}</Button>
          ) : <span className="text-xs text-muted-foreground">{tf("auto.589f9ad8d19d8b22")}</span>}
        </div>
      )}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {!readOnly && grant.capabilities?.canRevoke && (
          <div className="flex flex-wrap items-center gap-2">
            {<Button variant="outline" size="sm" onClick={onReconnect}><RefreshCw className="size-4" aria-hidden />{tf("auto.bf8a9eab9e7e141b")}</Button>}
            {account.status !== "revoked" && <Button variant="ghost" size="sm" className="text-destructive" onClick={() => setRevoking(true)}><Unplug className="size-4" aria-hidden />{tf("auto.c8534276405e474b")}</Button>}
          </div>
        )}
      </div>
      {revoking && <RevokeGrantDialog grant={grant} providerName={account.name} pending={revokePending} credentialPolicy={account.ownership === "shared" ? "shared" : "per_user"} isOwnIdentity={account.ownerUserId === currentUserId}
        description={tf("auto.103d4019f362a97a")}
        onCancel={() => setRevoking(false)} onConfirm={async () => { setRevokePending(true); setRevokeError(undefined); try { await onRevoke(); setRevoking(false); } catch (error) { setRevokeError(error instanceof Error ? error.message: tf("auto.e7071c1cd1dcb2d8")); } finally { setRevokePending(false); } }}>{revokeError && <p role="alert" className="text-sm text-destructive">{revokeError}</p>}{revocationDetails}</RevokeGrantDialog>}
    </section>
  );
}
